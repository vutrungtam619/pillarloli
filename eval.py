...
def do_eval(det_results, gt_results, CLASSES, saved_path):
    ...
    for id in ids:
        ...
        gt_dimensions = gt_result['dimension'].astype(np.float32)
...
def main(args):
    ...
    val_dataloader = get_val_dataloader(dataset=val_dataset, batch_size=args.batch_size, num_workers=args.num_workers)
    ...
    results_pkl_path = os.path.join(saved_path, 'results.pkl')

    # --- Nếu đã có submit + results.pkl thì bỏ qua predict ---
    if os.path.exists(results_pkl_path) and len(os.listdir(saved_submit_path)) > 0:
        print("Found existing predictions. Skipping inference...")
        with open(results_pkl_path, "rb") as f:
            format_results = pickle.load(f)
    else:
        print("No predictions found. Running inference...")
        model = Pointpillars().to(device)
        checkpoint_dict = torch.load(args.ckpt)
        model.load_state_dict(checkpoint_dict['checkpoint'])

        pcd_limit_range = np.array(config['pc_range'], dtype=np.float32)
        ...
        with torch.no_grad():
            ...
            for i, data_dict in enumerate(tqdm(val_dataloader)):
                ...
                for j, result in enumerate(batch_results):
                    ...
                    calib_info = batched_calibs[j]
                    tr_velo_to_cam = calib_info['Tr_velo_to_cam'].astype(np.float32)
                    r0_rect = calib_info['R0_rect'].astype(np.float32)
                    P2 = calib_info['P2'].astype(np.float32)
                    image_shape = data_dict['batched_image_shape'][j]
                    idx = data_dict['batched_idx'][j]
                    ...
                    for lidar_bbox, label, score, bbox2d, camera_bbox in \
                        ...
                        format_result['dimension'].append(camera_bbox[3:6])
                    ...
                    format_results[idx] = {
                        ...
                        'occluded': np.array(format_result['occluded'], dtype=np.int32).reshape(-1),
                        'alpha': np.array(format_result['alpha'], dtype=np.float32).reshape(-1),
                        'bbox': np.array(format_result['bbox'], dtype=np.float32).reshape(-1, 4) if len(format_result['bbox']) > 0 else np.zeros((0, 4), dtype=np.float32),
                        'dimension': np.array(format_result['dimension'], dtype=np.float32).reshape(-1, 3) if len(format_result['dimension']) > 0 else np.zeros((0, 3), dtype=np.float32),
                        'location': np.array(format_result['location'], dtype=np.float32).reshape(-1, 3) if len(format_result['location']) > 0 else np.zeros((0, 3), dtype=np.float32),
                        'rotation_y': np.array(format_result['rotation_y'], dtype=np.float32).reshape(-1),
                        'score': np.array(format_result['score'], dtype=np.float32).reshape(-1)
                    }
            write_pickle(results_pkl_path, format_results)
