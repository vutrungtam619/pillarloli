...
def anchor_target(batched_anchors, batched_gt_bboxes, batched_gt_labels, assigners, nclasses):
    ...
    return: 
           dict = {batched_labels: (bs, n_anchors),
    ...
    for i in range(batch_size):
        ...
        for j in range(n_assigners):  # multi anchors
            ...
            # Nếu không có GT → toàn bộ anchor là background
            if gt_bboxes.numel() == 0:
                assigned_gt_labels = torch.zeros(cur_anchors.shape[0], dtype=torch.long, device=anchors.device) + nclasses
                ...
                # c. ensure each gt has at least one anchor
            ...
            # reshape để nối lại multi anchors
        ...
        # concat tất cả anchors (multi-scale, multi-rotations)
    ...
    rt_dict = dict(
        batched_labels=torch.stack(batched_labels, 0),             # (bs, n_anchors)
        batched_label_weights=torch.stack(batched_label_weights, 0),
        batched_bbox_reg=torch.stack(batched_bbox_reg, 0),         # (bs, n_anchors, 7)
